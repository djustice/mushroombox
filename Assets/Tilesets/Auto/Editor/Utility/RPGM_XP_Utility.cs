...
namespace IGL_Tech.RPGM.Auto_Tile_Importer
{
    public class RPGM_XP_Utility
    {
        ...
        public static void Generate_Water_Tile_SS(string source_File_Path, string destination_dir_for_auto_tile,
            string frame1, string frame2, string frame3, string frame4,
        Dictionary<byte, int> rule_tiles, int wBlock, string asset_name)
        {
            ...
            if (atile != null)
            {
                ...
                var vars4 = Tiles_Utility.LoadSpriteSheet(frame4);
            ...
            Texture2D[] top_left_mini_tiles, Texture2D[] top_right_mini_tiles, Dictionary<byte, int> rule_tiles)
        {
            ...
            foreach (var comb in Tiles_Utility.ByteCombination())
            {
                ...
                int bl = Select_Mini_Tile(comb, 6, 5, 4, 0, 2, 1);
                int br = Select_Mini_Tile(comb, 4, 3, 2, 6, 0, 7);
                int tr = Select_Mini_Tile(comb, 2, 1, 0, 4, 6, 5);
                int tl = Select_Mini_Tile(comb, 0, 7, 6, 2, 4, 3);
