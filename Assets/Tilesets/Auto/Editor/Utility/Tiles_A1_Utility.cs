    ...
    public class Tiles_A1_Utility
    {
            ...
            Texture2D[] top_left_mini_tiles, Texture2D[] top_right_mini_tiles, Dictionary<byte, int> rule_tiles)
        {
            ...
            foreach (var comb in Tiles_Utility.ByteCombination())
            {
                ...
                int bl = Select_Mini_Tile_A1Water(comb, 6, 5, 4, 0, 2);
                int br = Select_Mini_Tile_A1Water(comb, 4, 3, 2, 6, 0);
                int tr = Select_Mini_Tile_A1Water(comb, 2, 1, 0, 4, 6);
                int tl = Select_Mini_Tile_A1Water(comb, 0, 7, 6, 2, 4);
        ...
        public static List<Texture2D> Generate_Final_Tiles_A1_Twister(int mini_tile_w, int mini_tile_h, Texture2D[,] left_mini_tiles, Texture2D[,] right_mini_tiles, Dictionary<byte, int> rule_tiles)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                foreach (var comb in Tiles_Utility.Left_Right_Combination())
                {
                    ...
                    #region DO NOT TOUCH! This is the magic
                    int l = Select_Mini_Tile_A1_Twister(comb, 6);
                    int r = Select_Mini_Tile_A1_Twister(comb, 2);
        ...
        /// <param name="rule_tiles"></param>
        /// <param name="wBlock"></param>
        public static void Generate_A1_Water_Tile_SS(string source_File_Path, string destination_dir_for_auto_tile, string frame1, string frame2, string frame3,
        Dictionary<byte, int> rule_tiles, int wBlock, string asset_name = "")
        {
            //create the auto tile for the animation water 
            //string dir = Path.Combine(Tiles_Utility.Auto_Tile_Folder_Path, string.Format(@"_{0}", Path.GetFileNameWithoutExtension(source_File_Path)));
            string dir = destination_dir_for_auto_tile;
            string atile_path = Path.Combine(dir,
                string.Format("{0}.asset", asset_name != "" ? asset_name : Path.GetFileNameWithoutExtension(frame1)));
            ...
            if (atile != null)
            {
                ...
                var vars1 = Tiles_Utility.LoadSpriteSheet(frame1);
                var vars2 = Tiles_Utility.LoadSpriteSheet(frame2);
                var vars3 = Tiles_Utility.LoadSpriteSheet(frame3);
        ...
        /// <param name="rule_tiles"></param>
        /// <param name="wBlock"></param>
        public static void Generate_A1_Twister_Tile_SS(string source_File_Path, string frame1, string destination_dir_for_auto_tile, Dictionary<byte, int> rule_tiles, int wBlock,
            string asset_name = "")
        {
            //create the auto tile for the animation water 
            ...
            Dictionary<byte, int> rule_tiles, int wBlock)
        {
            ...
            string atile_path = Path.Combine(Tiles_Utility.Auto_Tile_Folder_Path,
                string.Format("_{0}", Path.GetFileNameWithoutExtension(source_File_Path)),
                string.Format("{0}.asset", Path.GetFileNameWithoutExtension(frame1)));
            //string dir = string.Format(@"{0}/_{1}", Tiles_Utility.Auto_Tile_Folder_Path, Path.GetFileNameWithoutExtension(source_File_Path));
            string dir = Path.Combine(Tiles_Utility.Auto_Tile_Folder_Path, string.Format(@"_{0}", Path.GetFileNameWithoutExtension(source_File_Path)));
            if (!Directory.Exists(dir)) Directory.CreateDirectory(dir);
        ...
        public static void Generate_A1_Twister_Tile(string source_File_Path, string tiles_folder, Dictionary<byte, int> rule_tiles, int wBlock)
        {
            ...
            string atile_path = Path.Combine(dir, string.Format(@"{0}.asset", Path.GetFileNameWithoutExtension(tiles_folder)));
