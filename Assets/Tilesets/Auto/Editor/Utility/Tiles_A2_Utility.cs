...
namespace IGL_Tech.RPGM.Auto_Tile_Importer
{
    public class Tiles_A2_Utility
    {
        ...
        public static void Generate_A2_Tile_SS(string source_File_Path, string tile_path, string destination_dir_for_auto_tile,
            Dictionary<byte, int> rule_tiles, string asset_name = "")
        {
            //create the auto tile asset file
            ...
            string atile_path = Path.Combine(dir, string.Format(@"{0}.asset",
                asset_name != "" ? asset_name : Path.GetFileNameWithoutExtension(tile_path)));
